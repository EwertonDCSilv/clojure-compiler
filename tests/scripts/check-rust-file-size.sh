...
mkdir -p "$fixture_root/crates/demo/src" "$fixture_root/crates/demo/tests"
printf '%s\n' 'fn main() {}' >"$fixture_root/crates/demo/src/lib.rs"
printf '%s\n' 'fn test() {}' >"$fixture_root/crates/demo/tests/e2e.rs"

cat >"$fixture_root/baseline.json" <<'EOF'
{
  "version": 1,
  "limits": {"production": 800, "facade": 500, "test": 1000, "generator": 1200},
  "allowlist": {"crates/demo/src/lib.rs": {"lines": 1, "owner": "#109", "reason": "fixture debt"}},
  "exclusions": ["crates/demo/tests/e2e.rs"]
}
EOF
...
printf '%s\n' 'fn grew() {}' >>"$fixture_root/crates/demo/src/lib.rs"
